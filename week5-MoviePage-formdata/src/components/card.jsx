...
const Card = (props) => {
    ...
    return (
        <Item onClick={() => navigate(`/movies/${props.movie.id}`, {
            replace: false,
            state: { movieId: props.movie.id, title: props.movie.title}
          })}>
            <ImageBox>
                <S.StyledCard src={`https://image.tmdb.org/t/p/original/${props.movie.poster_path}`}>
                </S.StyledCard>
            </ImageBox>
            <MovieText textsize='0.9rem' margin='15px' weight='bold'>{props.movie.title}</MovieText>
